const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5209';
...
const adminCookie = await login('admin@ai-freelancer-agency.local', 'admin123');
const managerCookie = await login('manager@ai-freelancer-agency.local', 'manager123');
const analystCookie = await login('analyst@ai-freelancer-agency.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/proposals', analystCookie);
...
const records = await expectJson('/api/entities/proposals', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/proposals/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/proposals/approve`, {
  method: 'POST',
  headers: {
...
console.log('AI Freelancer Agency Operations Suite smoke passed');
